    ...
    <div className="animate-in fade-in duration-500 w-full mb-8">
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-6 mb-10">
        ...
        <div className="md:text-right flex flex-col md:items-end">
          <div className="flex items-center gap-2 text-[10px] font-extrabold text-brand-secondary uppercase tracking-widest mb-1">
        ...
        <div className="col-span-1 lg:col-span-8">
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm rounded-sm p-8 pb-10 transition-colors">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-12 text-center sm:text-left">
              <div className="w-28 h-28 bg-slate-900 dark:bg-slate-800 rounded-sm relative overflow-hidden flex items-center justify-center shrink-0 ring-1 ring-slate-100 dark:ring-slate-700 shadow-inner">
                 {/* Representing the avatar drawing visually simply */}
                 <User size={64} className="text-slate-700 dark:text-slate-300" />
                 <div className="absolute bottom-2 text-[8px] text-slate-500 font-bold tracking-widest bg-slate-900/60 w-full text-center py-1">CURATOR</div>
              </div>
              <div>
                <h2 className="text-2xl font-bold text-slate-950 dark:text-white capitalize">{user?.name || 'Guest User'}</h2>
                <div className="text-sm font-medium text-slate-500 dark:text-slate-200 mb-3">{user?.email || 'guest@example.com'}</div>
      ...
      {/* Account Actions Full Width Bottom */}
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm rounded-sm p-6 lg:px-8 transition-colors flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 lg:mt-8">
         <div className="flex items-center gap-2 text-[11px] font-bold text-red-500 uppercase tracking-wider">
           <span className="text-red-500 text-lg">&#9888;</span> ACCOUNT ACTIONS
        </div>
        <button 
          onClick={handleLogout}
          className="w-full sm:w-auto shrink-0 flex items-center justify-center gap-2 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/50 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30 hover:border-red-300 dark:hover:border-red-800 font-bold text-[11px] uppercase tracking-wider px-12 py-3.5 rounded-sm transition-all shadow-sm"
        >
          <LogOut size={16} />
          {t('logout') || 'Sign Out'}
        </button>
