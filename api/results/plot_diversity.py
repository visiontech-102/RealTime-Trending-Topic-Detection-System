...
rows = []
with open("metrics/coherence_diversity.csv", encoding="utf-8") as f:
    for r in csv.DictReader(f):
        rows.append(r)

model_keys   = ["lda", "nmf", "bertopic"]
model_labels = ["LDA", "NMF", "BERTopic"]

data = {(r["model"], r["language"]): float(r["diversity"]) for r in rows}
...
for bar in bars_en:
    ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.008,
            ...
            fontsize=8.5, color=COLOR_EN, fontweight="bold")
for bar in bars_so:
    ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.008,
            f"{bar.get_height():.4f}", ha="center", va="bottom",
            fontsize=8.5, color="#0369a1", fontweight="bold")

ax.set_xticks(x)
ax.set_xticklabels(model_labels, fontsize=12, fontweight="bold")
ax.set_ylabel("Topic Diversity Score", fontsize=11)
ax.set_ylim(0, 1.05)
ax.set_title("Topic Diversity Comparison\n(LDA vs NMF vs BERTopic — English & Somali)", fontsize=12, fontweight="bold", pad=14)
ax.axhline(0.7, color="gray", linestyle="--", linewidth=0.8, alpha=0.5, zorder=2)
ax.text(2.55, 0.712, "0.7 threshold", fontsize=7.5, color="gray", alpha=0.8)
...
print("Saved: diversity_comparison.png  /  diversity_comparison.pdf")
plt.show()
